...
- **The Deposit Approvals queue kept offering a live "Approve" button on a submission that could only ever fail.** A lead with two open deposit screenshots had its first one approved (converting the lead, locking its balance), but the second still showed "Approve" in Ehsan's queue, since the button's own logic only ever checked the document's status, never the lead's actual current state. Clicking it threw a real but confusing database error - "the lead is 'converted', not Pending Approval" for one real lead, "This lead's balance is locked after approval" for another, live on 2026-09-05. Fixed by checking the lead's real state at render time (the code to do this already existed, it was just never called before the click) and by adding a check for balance_locked, which had never been checked anywhere before.
