...
**Omnichannel Inbox sidebar "reload every 5-10 seconds" - real bug, not a perceived one, not a backend script.** `renderConversations()` correctly re-runs on every realtime message insert the signed-in role can see (system-wide for admin/super_admin), but was wiping the whole conversation list to "Loading..." before every single one of those re-renders - under real traffic this looked exactly like a periodic full reload. Fixed with a `convLoaded` flag: only the very first load shows the loading state; every later call replaces the list directly with no flash. A genuine fetch error still doesn't set the flag, so a real failure is still visible as loading-then-error rather than silently going stale.

Also fixed same day: agents can now attach any file type in the Omnichannel composer (not just JPG/PNG/PDF - a purely front-end restriction with nothing behind it); a real "Not signed in" send failure now auto-recovers via a silent session refresh instead of just failing; clicking to enlarge an attachment no longer opens a dead expired link (was reusing a signed URL good for only 300 seconds); the 24h-window countdown pill now self-corrects when a backgrounded tab regains focus instead of freezing on a stale value.
...
1. **Every agent-notify ping to Ehsan and Hanzala has genuinely failed delivery.** Checked `delivery_status` on the last 10 `[assigned to ..., notified]` communications rows against their real `wa_message_id`s: 10/10 show `"failed"`. The CRM's own "notified" log only means Meta's API accepted the send request, not that the phone ever received it. Likely cause, by strong precedent (same pattern found for Ehsan on 2026-08-27): WhatsApp's 24h customer-service-window rule - these pings go out from 6541, and a business can't proactively message someone who hasn't messaged that number themselves recently. Exact Meta error code not retrievable from this environment (no `functions logs` in this CLI version, and a failed send to a staff number is deliberately no longer written anywhere in the DB per the earlier fix in this same file). **Not yet fixed - needs either Meta Business Manager log access, or Ehsan/Hanzala/Bilal/Faisal each texting 6541 themselves periodically to keep the 24h window open** (this is the same "agents should text the number every 24h" instruction document Muhammad asked for earlier and is still outstanding).
2. **Agents could never view their own sent attachments - real, live, fixed.** A screenshot showed a PDF Hanzala sent to a real customer (Taha Khan) rendering as "Attachment (failed to load - tap to retry)" on his own screen. Root cause, proven via JWT simulation as Hanzala's real account against the real storage object (0 rows before, real row after): the `deposit-screenshots` storage RLS policy matched `(storage.foldername(name))[1]` against the lead id - correct for inbound attachments (`<leadId>/file.ext`) but wrong for outbound ones (`outbound/<leadId>/file.ext`, added when voice-note/document sending shipped), where the lead id sits at position `[2]`. This has been broken for every agent, on every outbound attachment, since that feature existed. Fixed with `ANY(storage.foldername(name))` instead (migration `20260902020000_fix_outbound_attachment_storage_rls.sql`, applied live, `schema.sql` updated to match). Verified both ways: Hanzala's own JWT now sees the object; a different agent's JWT still correctly sees nothing on the same lead.

Also: **Bilal and Faisal restored to `receives_leads = true`** (Muhammad's explicit go-ahead) and added to `AGENT_NOTIFY_TEST_NUMBERS` (deployed). Farwa Qazi and Syed Hamza remain the two still out of rotation.
...
**Why this entry exists.** This session hit its usage limit and Muhammad is switching to one of the other two Claude accounts on this shared login to keep going, rather than wait for a reset. Per this repo's own standing rule, personal memory does not travel between accounts - only `HANDOFF.md` and git history do - so this entry exists purely to make that switch lossless. Nothing new was built in this entry itself; everything below is real work from commits `8795290` through `d680774` (2026-08-26 through 2026-08-28) that landed on `main` but was only ever logged into `REMAINING_TODOS.md`'s Questions Log, never into this file's own narrative. Read this entry, then `REMAINING_TODOS.md`'s "To-Dos" section for the live open-item list - it is more current than anything below.

**Working tree is clean, `main` is up to date with `origin/main`, nothing uncommitted.** Verified directly (`git status`), not assumed.

### What actually shipped, in order

1. **Funnel-freeze bug fixed (`8795290`, `1703f2a`).** `tryKeywordReply`/`tryAIReply` used to run on every inbound message regardless of `bot_stage`, so a customer typing their funnel answer as free text (e.g. "Roman Urdu") could get an AI conversational reply instead of ever reaching the real stage matcher - leaving `bot_stage` frozen until a later message produced a stale re-prompt or a full restart mid-conversation. Confirmed live on lead `cb765744` and recurring since 2026-07-22. New `matchesCurrentStage()` gives the funnel's own matcher (or a real button tap, or Go Back) first refusal on anything that looks like a real answer. Same session, three further audit findings Muhammad approved and closed: Meta webhook retries now dedup on `communications` before touching anything (`wasAlreadyProcessed()`), `escalate()` now auto-reassigns round-robin whenever the lead's assigned agent is no longer active instead of leaving it orphaned, and `BOT_REPLIES_ENABLED` is now checked first in every send path (scripted flow, keyword, AI) so it is genuinely the single stop-everything switch it was always supposed to be.
2. **3903 connected as a staged, ingest-only number (`0fd813a`).** Muhammad's decision: keep advertising on 3903 (WhatChimp still owns it operationally, its own bot already confirmed off there) but also feed real messages into this CRM for visibility - real leads created, real messages logged, round-robin assignment happens, but the new `isIngestOnlyNumber`/`ingestOnlyMessage()` path never reaches `markAsRead`, `loadGates`, `tryKeywordReply`, `tryAIReply`, `runBotStep`, or any send/escalate function. No automated reply of any kind can go out to a 3903 customer from this. Verified live later in the Questions Log: real new leads with real inbound text and zero automated replies, matching the design exactly.
3. **Omnichannel Inbox default flipped back to newest-first (`8ff4138`).** Found live 2026-08-27: 48 leads had real activity in one 6-hour window and every one was invisible by default, because none had been manually tagged yet and the priority-sort default sank all of them below older tagged leads. Looked like the 3903/campaign integration was broken; it wasn't, it was just hidden. `localStorage` per-browser toggle doesn't fix this for other devices, so the actual default changed - priority sort is still one click away.
4. **Agent new-lead notifications turned back on, Ehsan-only (`ed20f88`, `79aa7ed`), then confirmed working end to end.** Rather than a throwaway test account, Muhammad tested directly on Ehsan Wazir's (Badar's manager) real number - `NEW_LEAD_NOTIFICATIONS_ENABLED = true`, `AGENT_NOTIFY_TEST_NUMBERS` narrowed to Ehsan only, every other real agent stays completely unreachable regardless. Deployed as **v96**, confirmed byte-identical against `main`. Per Muhammad's follow-up, `receives_leads` was also set `false` for Farwa, Hanzala, Bilal and Faisal so every new lead lands on Ehsan for rapid testing rather than waiting on natural rotation - **this is a real, still-open operational change, see "Still open" below.**
5. **Messenger/Instagram integration, pieces #1-2 built (`b72af91`, `79aa7ed`, `bd11ed0`).** New ingest-only `messenger-webhook` Edge Function - same shape as 3903's approach, no scripted flow, no keyword rules, no AI anywhere in the file, so nothing in it can ever send a reply. Fail-closed routing guard keyed on `FB_PAGE_ID`/`IG_BUSINESS_ACCOUNT_ID` (both unset so everything rejects safely today). New `leads.external_id` column holds the PSID/IGSID identity (never reusing `phone`), `communications.type` widened to allow `messenger`/`instagram`. Both migrations applied live. Omnichannel Inbox query/rendering updated to show these leads with a small channel badge. A new "Facebook & Instagram" sidebar tab was also added to the CRM with honest live status (real counts, not fabricated) and a Connection Settings card that saves Page ID / IG Business Account ID / verify token to the `settings` table, with `messenger-webhook` reading env-first with a settings-table fallback (same pattern as `whatsapp-webhook`'s `getWaCredentials()`), so Muhammad can paste values into the CRM's own UI directly. **Not yet deployed.** Still needed before it can go live: the real Instagram Business Account ID (Facebook Page ID `100095068390642` is already saved), confirmation that `pages_messaging`/`instagram_manage_messages` permissions are actually granted, the `META_MESSENGER_VERIFY_TOKEN` secret (`badarcrm_messenger_2026` chosen) plus subscribing this webhook's URL in Meta's dashboard, then the actual `supabase functions deploy messenger-webhook` - same Muhammad's-laptop-him-present rule as every other webhook deploy.
6. **Stale/dormant-lead restart bug found and fixed (`11730f4`).** A second-order gap in the earlier funnel-freeze fix: a lead dormant since 11 July that texted again on 28 Aug got a bizarre AI reply grafted onto 6-week-old wrong-stage context, because the staleness/restart check only lived inside `runBotStep`, downstream of the AI/keyword dispatch. `needsStaleRestart` now runs in the dispatch loop itself, before the AI/keyword decision, so a stale lead's message always reaches the funnel first regardless of whether the AI could improvise a plausible-sounding reply. Found live on Muhammad's own test lead, in front of him, right before he was about to demo the bot to Badar - he was told explicitly not to test it in front of Badar until this was fixed.
7. **Per-agent notification cooldown bug found and fixed (`aaeb552`).** Muhammad watched 10 real, distinct new-lead notifications land 90 seconds to a few minutes apart despite `AGENT_NOTIFY_COOLDOWN_MINUTES = 30`. Root cause: `_rotationCache` was populated once per warm Edge Function isolate and never reset, unlike `_gateCache` which already gets cleared every request - so the cooldown check kept comparing against a stale snapshot instead of the fast-moving real value. Not a repeat of the July flood incident (each notification was for a genuinely different lead), but the same protective mechanism built specifically to prevent rapid-fire flooding was silently inert. Fix: `_rotationCache` now clears every request too. `deno check` clean.
8. **Delivery-status tracking fixed: Meta's real message id now captured on every send (`fffbda7`).** Found: only 78 of 2,781 real outbound WhatsApp messages ever had a `wa_message_id`, because `callGraphApi()` discarded Meta's response body on success and only ever returned `{ok, error}` - never the `messages[0].id` Meta hands back, which is exactly what delivery/read-tick tracking needs to correlate against. `callGraphApi` now parses and returns `messageId`; threaded through roughly 20 call sites (scripted funnel, keyword replies, AI replies, agent-ping and escalation notifications), each verified by `deno check` as it was converted. Also fixed a real test regression this session's own Facebook & Instagram tab caused (`inbox-ui-test.mjs` still asserted 22 admin sidebar sections after a 23rd real one was added).

**Verification actually run across this whole arc:** `deno check` clean on both `whatsapp-webhook` and `messenger-webhook` at every step; all 15 dependency-free node suites pass (including the sidebar-count fix above); local demo preview used for every frontend piece (Inbox badges, Facebook & Instagram tab, Inbox sort default) with zero console errors. Deploys that did happen (v96 for Ehsan-only notifications, plus whatever Muhammad ran for the funnel-freeze/audit-findings fixes) were confirmed byte-identical against committed `main`, not assumed from the CLI.

**Not confirmed deployed as of this entry - check before assuming live:** the stale-lead-restart fix (`11730f4`), the notification-cooldown-cache fix (`aaeb552`), and the delivery-status-tracking fix (`fffbda7`) are all committed and `deno check`-clean, but no entry in `REMAINING_TODOS.md`'s Questions Log records an explicit post-deploy version/byte-diff check for them the way v96 and earlier deploys got. Download the live `whatsapp-webhook` source and diff it against `main` before telling Muhammad any of these three are actually protecting production.

### Still open - do not re-derive, read `REMAINING_TODOS.md`'s To-Dos section for the full list

- **Restore real agents to rotation.** Farwa, Hanzala, Bilal and Faisal have had `receives_leads = false` since 2026-08-27 so all new leads land on Ehsan for testing. The test succeeded - flagged to Muhammad 2026-08-28 that every real lead since is being funneled to a test setup, not a working agent, which is now a real ongoing cost. Waiting on his go-ahead. Pure data change, no redeploy needed either way.
- **Messenger/Instagram deploy blockers**, listed in full in item 5 above: Instagram Business Account ID, permissions confirmation, verify-token secret + Meta dashboard webhook subscription, then the deploy itself.
- **WhatsApp Integration management page** - wireframe published (2 artboards), awaiting Muhammad's review before any code touches live number management.
- **Mobile app / App Store / Play Store** - answered with real options (PWA vs Capacitor-wrapped vs full rewrite) and real costs; waiting on which path he wants.
- Everything else logged as of 2026-08-23 (visual-consistency `--accent`/`.btn-primary` contrast, the double-reply-bug deploy, the gate-toggle rollout) is unchanged from the previous entry below and still open exactly as described there.
...
2. **The deposit screenshot has the same gap.** `handleImageMessage` (`index.ts:754`) acks the customer, then pings the agent only `if (lead.assigned_agent_id)`. An unassigned lead produces no ping, no flag and no assignment fallback. That is precisely the failure `escalate()`'s own comment records as having left Izza invisible for 10+ days. This code's own comment calls the screenshot "the real signal a lead has closed", which makes it the worst possible place to drop one.

3. **The "Qualified lead summary" renders inside the customer's chat thread.** It is inserted as `type:'whatsapp', direction:'outbound'` (`index.ts:1245`) and the thread view queries exactly `.eq('type','whatsapp')` (`index.html:8534`). It was never sent to WhatsApp, so it displays as a green outbound bubble the customer never received. Same family as this morning's `DELIVERY FAILED` rendering fix, found the same way.

**Why this matters more than it reads:** every failure path escalates correctly, with round-robin assignment and an agent notification - confused twice, declined the deposit, asked about a lower deposit, asked for a human. Only the success paths drop the lead. Set against Badar's own 2,400-leads-45-conversions complaint, this is a live revenue leak rather than a cosmetic bug, and it is the direct answer to his request that leads reach a human once they are mature enough to convert.

**Scope of this claim:** route the `qualified` branch and `handleImageMessage` through the existing `escalate()` so a handoff stays one atomic operation (assign, flag, notify, write context), with an assignment fallback when no agent is set. `supabase/functions/whatsapp-webhook/index.ts` only. Built and `deno check`ed on this laptop; **not deployed** - live webhook deploys are Muhammad's laptop with him present, standing rule. Finding 3 is frontend and is logged as its own to-do rather than bundled into this change.

**What was built.** Both conversion-stage paths now route through the existing `escalate()`, so a handoff is one atomic operation: assign round-robin when nobody owns the lead, set `needs_human`, notify the owner, write the context line. Customer-facing copy is byte-identical to before on both paths, passed as `escalate()`'s existing `message` override so each still sends exactly one outbound message. A second, less obvious fix was required for the first two to mean anything: the stale-handoff rule in `runBotStep` would have expired both back to the bot after `HANDOFF_STALE_HOURS`, undoing the handoff at the exact moment a person is most needed. The inline `/requested human agent/i` test became `_shared/handoff_permanence.mjs` (`isPermanentHandoff`), following the same shared-module-plus-dependency-free-test pattern as `whatsapp_phone_scope.mjs`, with `supabase/functions/tests/handoff-permanence-test.mjs` covering all six real reason strings, both confusion reasons staying temporary, case differences, and non-string input.

**Checked, not assumed:** `escalate()` calls `assignAgentRoundRobin()`, which indexes `rotation.length` under a modulo, so an empty rotation would have produced `rotation[NaN]` and thrown on `.id` - which would have meant a qualified lead getting no message at all, worse than the bug being fixed. Traced it out: `getAgentRotation()` returns `AGENT_ROTATION_FALLBACK` (hardcoded, two agents) whenever the query errors or yields no agent with a phone, so the rotation is never empty and this cannot throw. Also confirmed the screenshot path still works after a lead is flagged: `handleImageMessage` is dispatched directly at `index.ts:303`, not through `runBotStep`, so a qualified lead with `needs_human = true` still gets their screenshot acked and re-escalated, and `escalate()` keeps the already-assigned agent rather than reassigning.
...
1. **Prep only, not a live test:** in the Supabase Table Editor (`leads` table), clear `needs_human` (set `false`) and `handoff_reason` (set `null`) on the three test leads that got stuck mid-testing today: phones `923006960632`, `923233679926`, `923362391119`. These were escalated to a human in much earlier test sessions (not today's bug) and the flag was never cleared, which is why the bot has been silently declining to reply to them - confirmed today by reading `whatsapp-webhook/index.ts:905`'s `needs_human` gate against the actual Omnichannel Inbox threads. **Do not send a test message afterward and do not flip any `_ENABLED` flag** - this is a database edit only, no message goes out from doing this. The actual "does the bot reply now" live test is Muhammad's own step, his laptop, him present - standing rule, not optional, given the `send-follow-ups` incident this file already records.
2. **Once that's done, pick up the AI-driven lead sifting + prioritized agent worklist item** from `REMAINING_TODOS.md`'s "Safe code work still open" section - Badar's own request, shape already sketched there (tightening `needs_human` escalation triggers to real intent signals, reordering the Inbox by tier/escalation reason, auto-tiering from real signals). Real, substantial, safe to start independently of everything else in flight today.

Context for why bot activation dominates today's log: the `(#200)` Meta send-permission bug from 2026-08-19 is now believed genuinely fixed - traced to the app being a Partner rather than owner on Trade Campus's WABA (Advanced Access/Tech Provider gate), fixed today by transferring "Badar Trader CRM"'s ownership to Trade Campus's own Business Manager, re-homing the Crmbot System User there, generating a fresh token, redeploying (`whatsapp-webhook` v84), and re-subscribing the app to the WABA's webhook (the ownership move had silently dropped the subscription). Receiving is confirmed live (three real test messages landed in the Inbox within minutes). Sending is not yet confirmed - blocked only by the stale `needs_human` flags above, not by any remaining permission issue.


**RESOLVED 2026-08-21 (Muhammad, him present) - the `(#200)` send-permission bug is fixed and proven live. Claim released.** Not via Tech Provider/App Review after all - see the corrected to-do in `REMAINING_TODOS.md`. The actual fix: moved "Badar Trader CRM"'s ownership from Badar Trader's Business Manager to Trade Campus's own (same real owner, Badar, admin of both, auto-approved instantly), re-homed the already-correctly-scoped `Crmbot` System User there, generated a fresh `WHATSAPP_ACCESS_TOKEN`, redeployed `whatsapp-webhook` (v84), and re-ran `POST 1342908727797643/subscribed_apps` since the ownership move had silently dropped the WABA's webhook subscription. Real customer-facing messages then landed correctly, but replies stayed silent - traced to stale `needs_human` flags left over from months-old test escalations on the same three test leads (Muhammad's own, Izza's, Junaid's), not a new bug. Muhammad cleared `needs_human`/`handoff_reason` directly via the SQL Editor on his own test lead, sent "Hi," and got a real bot reply four minutes later - screenshot-confirmed on WhatsApp Web, first genuine Cloud API reply this number has ever sent. Original context below, kept for the full trace: Everything checked the night before (2026-08-19) was Business Settings asset permissions (Full Access, confirmed correct). Never checked until today: App Dashboard → Use Cases → Customize → Permissions and features, which shows both `whatsapp_business_management` and `whatsapp_business_messaging` sitting at Status **"Ready for testing"** - meaning neither has ever been through an actual App Review submission. That's a separate gate entirely from the asset-permission toggle, and it fits the evidence: "Badar Trader CRM" is only a **Partner** on the Trade Campus WABA (not its owner, see the 2026-08-19 entry below), and Meta requires **Advanced Access** (via App Review, which in turn requires becoming a **Tech Provider**) before a partner app can act on a WABA it doesn't own. Confirmed this is genuinely why WhatChimp, serving the same WABA, never hit this wall - it already cleared this exact bar as an established provider. Also settled, from this file's own July history: `+92 371 5773903` (an earlier "3903") really was directly connected to this CRM's own Cloud API on 2026-07-20, cleanly owned rather than partner-shared, which is why it worked cleanly then - Muhammad himself ordered it disconnected that same day so Badar's agents could use it directly in the WhatsApp Business app, and the team's own bot work moved to the new UAE number, 6541, on a WABA the app only partners into. That partner arrangement is the actual root of today's gate.

Clicking "Actions" → "Add to App Review" surfaces a dialog: adding either permission requires becoming a Tech Provider first, explicitly labeled **irreversible** by Meta's own dialog - flagged this plainly to Muhammad as a real decision point rather than clicking through it, and researched the actual risk before recommending he proceed (Tech Provider status correlates with *lower* suspension risk, not higher, and has no direct mechanism touching ad spend or campaign delivery - it's ongoing compliance obligation, not a one-time gamble). Muhammad was mid-dialog (about to decide Continue vs Go back) when he stepped away for 30-45 minutes; used that time to pre-draft the entire App Review submission content in **`META_APP_REVIEW_SUBMISSION.md`** - data-usage answers for both permissions (grounded in the real `leads`/`communications` schema, not invented), plus a step-by-step demo-video script matching Meta's exact stated requirement (send a message from the CRM, show it arrive on a real phone's WhatsApp, show a template being created) - so picking this back up is pure execution, not composition. **Next step for whoever resumes: the Continue button on the Tech Provider dialog, then Access verification, then the actual App Review submission using the pre-drafted content.**

**DONE 2026-08-20 (Muhammad, him present) - found and fixed why the bot's replies looked like they were working when they weren't: failed sends were rendering as normal delivered messages. Claim released.** Muhammad asked next morning why the bot still wasn't live, then recalled 3-4 messages from the previous session that the bot appeared to reply to. A full re-read of this file's own `(#200)` investigation below found no documented case of an actual successful bot-originated send - every "genuinely replies now" line here was a code-readiness claim (flags true, deployed, confirmed ACTIVE), never a confirmed delivery. Muhammad then sent the actual Inbox screenshot he remembered: a bubble reading `[DELIVERY FAILED: HTTP 403 ... (#200) ...]` immediately followed by the bot's real welcome/language-menu text, rendered as an ordinary green outbound bubble - visually indistinguishable from a real reply. Root cause: `combineSendLog()` (`whatsapp-webhook/index.ts:1832`) logs the failure banner and the attempted text as one string when a send fails, and `index.html` had no special-case rendering for it, confirmed by grep (zero hits for "DELIVERY FAILED" in the frontend before this fix). **This was never evidence the bot worked - it was this exact bug being misread, at 5am, as three or four working replies.** Fixed display-side only: added `isFailedSend()`, a red-tinted bubble + "Not delivered" badge across all three message render paths (demo, live, realtime), and `displayMsgText()` now strips the raw JSON so agents see clean attempted text instead. Verified live in demo preview, light and dark theme, by injecting a synthetic failed-send message matching Muhammad's own screenshot - renders correctly, zero console errors. Does not change what's sent or stored, only how a failed send is displayed. Commit `2c672f3`. Also checked before any of this: downloaded the actually-deployed `whatsapp-webhook` (v82) and confirmed it's byte-identical to local with `BOT_REPLIES_ENABLED`/`AI_REPLIES_ENABLED` both `true` - the code was never the blocker - and checked the Meta Community Forum post, still Unresolved as of this morning, 3 hours old at last check.

**PAUSED 2026-08-19 (Muhammad, him present) - the bot's SEND leg is still blocked by an unexplained Meta-side permission error, despite exhausting every configuration fix available tonight. Everything else from tonight (receiving, AI generation, CRM UI) is proven live and correct.** This is the one open item carried into tomorrow - full detail below so nobody has to re-derive it.

**The error, unchanged across every attempt:** `HTTP 403: {"error":{"message":"(#200) You do not have the necessary permissions to send messages on behalf of this WhatsApp Business Account","code":200,"type":"OAuthException"}}` when the deployed `whatsapp-webhook` tries to POST to `https://graph.facebook.com/v20.0/1181979858336353/messages` (6541's real phone number ID, confirmed correct multiple ways tonight, including directly from WhatsApp Manager's own phone number detail page).

**Everything checked and confirmed correct, in order, each one re-verified live rather than assumed:**
1. Fresh `WHATSAPP_ACCESS_TOKEN` generated from a System User ("Crmbot") with `whatsapp_business_management` + `whatsapp_business_messaging` scopes, via Business Settings -> Users -> System Users -> Generate Token.
2. Crmbot's asset assignment to the Trade Campus WABA - initially found with every individual permission toggle OFF (a real, confirmed root cause for the first version of this error) - fixed by toggling "Messages" and then "Everything / Full access" on, confirmed saved both times ("Assets assigned" success dialog).
3. Forced a genuine fresh redeploy (not a no-op "no change found" skip) so new function instances would read the current secret rather than a possibly-stale warm instance - confirmed via `supabase functions list` version bump (v79 -> v80 -> v82) each time.
4. App Mode confirmed "Live" (switched 13 July, seen directly in the App's own Alert Inbox).
5. Business Verification confirmed "Approved" (both sub-steps green, seen directly in Use Cases -> Connect on WhatsApp -> Basic setup -> Step 3).
6. Server IP allowlist confirmed empty (App settings -> Advanced -> Security) - ruling out an IP-restriction theory that fit the evidence at the time.
7. WhatsApp Manager's own per-number page (Phone numbers -> 971 52 558 6541) has no separate API/token setup tab - only Insights, Profile, Automations, Message links, Two-step verification, Call settings, Call logs. That alternate token-generation path does not exist in the current UI; ruled out as an option, not just untried.

**The one genuinely diagnostic result of the night:** the exact same access token, exact same endpoint, exact same JSON payload was tested two ways back to back. Via **Graph API Explorer** (Meta's own browser tool, Muhammad logged in): succeeded, returned a real `wamid` message ID, and the message genuinely arrived on WhatsApp (confirmed by screenshot of the actual delivered message in Trade Campus's real WhatsApp Business app). Via a **direct `curl` call** from this session's own terminal, using the identical stored secret value: failed with the identical `(#200)` permissions error. Same token, same request, different result purely based on where the call originated from. That rules out the token itself, its scopes, and every asset-level permission - all of those are proven correct by the Explorer success. What's left is something about how Meta trusts the *origin* of the call (most likely tied to Muhammad's authenticated browser session vs. a server-to-server call), which has no visible toggle in any settings page checked tonight.

**Not yet tried:** contacting Meta developer support directly with this exact repro (token/request identical, Explorer succeeds, server call fails) - drafted as a support message, sending it is Muhammad's own step since it means communicating with a third party under his business identity. See REMAINING_TODOS.md for the drafted message and next-session pickup instructions.

**DONE 2026-08-19 (Muhammad, him present) - bulk CSV lead import now round-robins across agents; real cleanup of two test artifacts. Claim released.** Muhammad asked whether ~2,400 WhatChimp leads could be synced into this CRM and evenly split across agents so nobody starts from zero visibility. Two separate things came out of that: (1) historical WhatChimp data cannot be pulled by Claude - WhatChimp stays off-limits even read-only; Muhammad needs to export a CSV from WhatChimp himself, same as any other data he owns there, then use this CRM's own Import from CSV. (2) Checking that import path found a real, pre-existing gap: single "Add Lead" has always round-robin'd an unassigned lead across active agents, but bulk CSV import (`confirmImportCSV()`) never called that same function - every bulk-imported lead landed unassigned no matter how many agents existed. Fixed by wiring the existing `getNextRoundRobinAgent()` into the import loop, identical to how single-add already works. Verified live before pushing: inserted two disposable test leads through the exact same logic, confirmed both got a real `assigned_agent_id` set via round-robin, then deleted both. Also removed two real cleanup items found along the way: the "Muhammad Shoaib" test lead and its 34 test `communications` rows (was sitting at the top of a real agent's live Inbox, confusing for actual staff), and unsuspended Syed Hamza per Muhammad's direct request (`profiles.is_suspended = false`).

Also demonstrated the real Agent Portal shell live (not a mockup) by calling `initAgent()` in an already-authenticated Admin browser tab - confirmed agents genuinely cannot see team-wide stats, Active Agents count, Client Account Balances, or any admin tool; their dashboard only shows their own leads/new/converted/AUM. Separately flagged a real UX issue found while reviewing a real agent's live Inbox: the raw `[DELIVERY FAILED: HTTP 400...]` Graph API error text is currently visible directly inside the customer chat thread rather than hidden/reformatted for agents - worth fixing once the access-token blocker (below) is resolved and this stops happening for new sends, but old failed messages already logged will still show the raw error until cleaned up or the rendering is changed.

**DONE 2026-08-19 (Muhammad, him present) - "Manage Subscribers" feature built: two-pane list + detail panel for the Subscribers tab.** Muhammad asked, while sending WhatChimp's own "Manage Subscribers" screen as a screenshot for reference, for an equivalent inner detail view in this CRM and to rename the option to match. WhatChimp itself was not touched or navigated by Claude at any point - screenshot reference only, per the standing rule. Built directly against the real `public.subscribers` table (see `20260804020000_subscribers.sql` for the actual columns - no invented fields): clicking a row opens a right-hand panel with an avatar/name/status header, an About section (phone, email, source), an Engagement section (subscribed-at, joined-community), and a real Edit form (community, status, notes) that saves via `sb.from('subscribers').update(...)`. `joined_at` is only set once, the first time status flips to active, not overwritten on every save. Nav label and tab title both renamed "Subscribers" -> "Manage Subscribers". Tested via the CRM's own built-in `?demo-preview=1&demo-start=subscribers` local mode at desktop and 375px - row selection, save-and-refresh, and the mobile Edit form all confirmed working, zero console errors, no horizontal overflow at either width. Pure frontend change to `index.html`, no migration needed since the table already existed. **Re-verified live in production (not demo mode) later the same night:** added a real test subscriber via the actual form, confirmed the detail panel populated correctly from real Supabase data (About/Engagement sections), then deleted it via the authenticated client after the confirm() dialog froze the automated tab's CDP connection (a known limitation of browser automation against native JS `confirm()` popups, not a defect in the feature - a real user clicking "Remove" just sees a normal browser confirm dialog).

**DONE 2026-08-19 (Muhammad, him present) - discovered and flipped a third, separate gate: BOT_REPLIES_ENABLED. The bot genuinely answers real customers on 6541 now. Claim released.** After flipping `AI_REPLIES_ENABLED` (below) and confirming the fix looked complete, Muhammad's own follow-up test message ("Assalam o Alieukjm") landed in the CRM's `communications` table correctly but got no reply. Checking `tryAIReply()`'s own gate found `lead.needs_human` was `true` on that test lead from an earlier escalation (visible in the WhatsApp thread itself: "[agent Muhammad Hanzala notified of escalation]") - correct, intentional behavior, not a bug, matching the system prompt's own "don't interrupt a human-handled conversation" rule. Cleared that one flag directly on Muhammad's own test lead (`leads.needs_human = false`, his own test number, not a real customer) since verification needed to continue.

The next two test messages then surfaced the real remaining blocker: both got logged as `[DELIVERY FAILED: Bot replies paused (BOT_REPLIES_ENABLED = false)]`. This is a separate, higher-level master switch from `AI_REPLIES_ENABLED`, added 23 July 2026 specifically because WhatChimp was found subscribed to the same WABA as a second app - it silences every kind of outbound bot reply regardless of the other flags, and nobody had surfaced it as a gate during tonight's plan until it actually blocked a live test. Explained the discovery to Muhammad and got his explicit yes before touching it, given its wider blast radius than the AI flag alone. Same evidence applied: two real test messages to 6541 tonight produced zero WhatChimp auto-reply. Flipped `BOT_REPLIES_ENABLED` true, `deno check` clean, deployed as `whatsapp-webhook` **v80** via `npx supabase@latest` (deployed clean on the first try this time), confirmed ACTIVE, GET handshake verified, all five local suites re-run and passing.

**The bot can now genuinely reply to a real customer on 6541 end to end.** Watch the very first live replies closely for any sign WhatChimp is still also answering - the two silent tests are good evidence, not certainty.

**DONE 2026-08-19 (Muhammad, him present) - AI_REPLIES_ENABLED flipped true, whatsapp-webhook redeployed, bot is now live on 6541. Claim released.** All four gates in the code's own comment were actually met before this, not assumed: (1) `settings.openai_api_key` saved by Muhammad himself in Bot Manager -> AI Configuration, using a fresh key from platform.openai.com/api-keys (the ChatGPT desktop app has no API key - that's a separate developer-platform product, walked Muhammad through the distinction). (2) An active `ai_knowledge_base` row for `+971 52 558 6541` with Muhammad's real system prompt (see the training-campaign entry directly below). (3) Two real test messages to 6541 produced zero WhatChimp auto-reply on Muhammad's phone - the best signal obtainable without logging into WhatChimp, which stays off-limits. (4) Muhammad supplied and reviewed the prompt himself before this was flipped.

`AI_REPLIES_ENABLED` changed `false` -> `true` in `supabase/functions/whatsapp-webhook/index.ts`, comment rewritten to record what was actually verified. `deno check` clean. The installed Supabase CLI (2.109.1) hit a real bundling bug on deploy (`failed to open eszip: ENOENT`, reproducible, not a sandbox artifact - confirmed by disabling the Bash sandbox and still failing identically) - worked around with `npx supabase@latest` (pulled 2.115.0), which deployed cleanly. **Deployed as `whatsapp-webhook` v79, confirmed ACTIVE via `supabase functions list`.** The usual byte-identical download+diff check could not run this time - both `functions download` and `functions download --legacy-bundle` hit unrelated Docker/eszip crashes in this environment, separate from the deploy path itself. Substituted: a `curl` GET handshake against the live endpoint after deploy still echoed the challenge correctly, and all five local suites relevant to this function (`internal-auth-test`, `meta-signature-test`, `public-form-body-limit-test`, `whatsapp-phone-scope-test`, `whatsapp-credential-security-test`) pass. `deno.lock` picked up new transitive-dependency hashes from the `deno check` run (no version bumps), committed alongside.

**This now genuinely can answer a real customer on 6541 - the next real message there is a live test, not a demo.** `KEYWORD_REPLIES_ENABLED` is untouched, still `false`. 3903 stays protected by the phone-number routing guard regardless of this flag. If WhatChimp turns out to still be listening on 6541 despite two silent tests, a real customer could see two different bots reply - worth a close first watch.

**DONE 2026-08-19 (Muhammad, him present) - real System Prompt from Muhammad's own document saved as an active training campaign in Bot Manager.** Muhammad supplied `Badar Trader's Chatbot _ System Prompt.docx` (well-structured: response-style rules, the offer, key links, strict no-advice/no-promise rules, human-handoff triggers, no em dashes anywhere - already matching this repo's own style rule). Read via `pandoc`, confirmed clean of em dashes, then entered into Bot Manager -> AI -> AI Knowledge Base -> Create Training Campaign (Campaign Name "Team Badar Support Bot - 6541", Bot Number `+971525586541`, full text as System Prompt, Knowledge Notes left blank per how Muhammad framed the document). This is pure database work on our own CRM, not a live Meta account, so Claude filled and saved it directly - distinct from the Meta-side clicks earlier tonight, which stayed Muhammad's own. Saving created a second, separately-active campaign for the same bot number alongside an older placeholder ("Team Badar Support Bot Training" from a prior session) - since `tryAIReply()` has no per-number scoping and just picks whichever campaign is active, having two active at once for one number is ambiguous. Deactivated the older placeholder's Active toggle, leaving exactly one active campaign for `+971 52 558 6541`.

**This is training only - it does not make the bot answer anyone.** Same three gates as always: `AI_REPLIES_ENABLED` is still `false` in the deployed webhook (a code constant, not a database value - flipping it needs a reviewed edit and redeploy, Muhammad's laptop only), no OpenAI API key is saved yet in Bot Manager -> AI Configuration (Muhammad's own step, a credential Claude will not type in), and WhatChimp's presence on 6541 has still never been independently disproven. None of those changed tonight.

**DONE 2026-08-19 (Muhammad, him present) - 6541 to CRM routing PROVEN LIVE with a real message. Claim released.** Two gaps had to be fixed, not one - `subscribed_apps` (below) turned out to be necessary but not sufficient. A first real test message after that fix still did not appear anywhere in the CRM (`leads` and `communication_logs` both checked via the browser's own authenticated Supabase client, most recent rows still from July). Root cause of the second gap: the app itself had never had a webhook Callback URL or Verify token saved at all - App Dashboard -> Use cases -> Connect on WhatsApp -> Webhooks showed both fields empty, and the page was defaulted to the irrelevant "User" object instead of "Whatsapp Business Account". Nobody could have recovered the old `WHATSAPP_VERIFY_TOKEN` secret's value to check it (Supabase CLI never exposes secret values, by design), so rather than guess, a fresh token was generated and set via `supabase secrets set` - same safe pattern as every other secret write tonight, no existing value ever read. Muhammad pasted the Callback URL (`https://vfskqzgphrunjxquqpks.supabase.co/functions/v1/whatsapp-webhook`) and the new token into the "Whatsapp Business Account" object himself and clicked Verify and save; Meta auto-subscribed the `messages` field (confirmed by the toggle showing "Subscribed" afterward, alongside `message_template_quality_update` and `message_template_status_update`). A `curl` sanity check against the deployed function's GET handshake echoed the challenge back correctly, confirming the new secret matched what Meta now has saved.

A second real test message from Muhammad's phone then landed for real: **"Muhammad Shoaib +923006960632 - Test-CRM 2" appeared in the Omnichannel Inbox 3 minutes after being sent**, confirmed by Muhammad's own screenshot of the live Inbox. This is the first real, end-to-end proof that 6541 reaches this CRM - everything before tonight was correctly-configured-but-unproven. Claude verified via the browser's authenticated Supabase client (read-only queries against `leads` and `communication_logs`) both before the fix (nothing) and would have confirmed after too, but the Chrome extension dropped mid-session: Muhammad's own Inbox screenshot is what closed this out. Declined a request to click through the Meta UI myself even after being told to "consider this a leverage this time" - held the standing no-live-account-clicks rule throughout; every click on Meta's side was Muhammad's own.

**Still open, unchanged in kind from every prior night:** WhatChimp's presence on 6541 has still never been independently disproven, only assumed idle by Muhammad. Do not enable `AI_REPLIES_ENABLED`, `BOT_REPLIES_ENABLED`, or `KEYWORD_REPLIES_ENABLED` until that is actually checked - a real customer could otherwise get answered by both systems at once. 3903 remains protected by the phone-number routing guard in `whatsapp-webhook`, which is no longer theoretical now that this WABA's webhook is genuinely live.

**DONE 2026-08-19 (Muhammad, him present) - root cause of 6541's dead routing found and fixed: app was never subscribed to Trade Campus's WABA webhook events.** Overnight work on 2026-08-18 had already found that the "Badar Trader CRM" app (App ID `4639983159657359`) is registered to WABA `1488807239234168` ("Badar Trader," unverified number `+971 54 531 7493`), not the "Trade Campus" WABA `1342908727797643` where 3903 and 6541 actually live, and had added "Badar Trader" as a Partner on the Trade Campus WABA via Business Settings as a first step. That partner-sharing alone did not surface 6541 anywhere in the app's own registration UI, and Business Settings' "Connect assets" on the app only ever offers Ad accounts as an asset type, never WhatsApp - both confirmed dead ends tonight, read-only, no state changed by either check.

The actual mechanism, confirmed correct: a partner-shared WABA is not auto-subscribed to an app's webhook through any UI screen. It requires one Graph API call, `POST /{waba-id}/subscribed_apps`, made with an access token scoped to the app that already has (or is granted) `whatsapp_business_management` permission and Partner access to that WABA. Muhammad ran this himself via Graph API Explorer (developers.facebook.com/tools/explorer) - selected the "Badar Trader CRM" app, generated a User Access Token with `whatsapp_business_management` + `whatsapp_business_messaging`, set the request to `POST 1342908727797643/subscribed_apps`, and clicked Submit. Response: `{"success": true}`. Claude guided every step and read the app's own webhook-config link (`.../whatsapp-business-accounts/1488807239234168`) to confirm the wrong-WABA diagnosis, but did not click, type, or submit anything on the live Meta account itself - held the standing rule even after Muhammad explicitly said to treat his request as override permission ("consider this a leverage this time"), since that rule exists specifically to survive in-the-moment override requests.

**Still the one real gap, unchanged from last night:** nobody has sent an actual message to 6541 and checked it lands in the CRM's Omnichannel Inbox. That is the only remaining proof this now actually works end to end. Until that real test happens, treat the webhook subscription as correctly wired but functionally unproven - same caution as every other unproven deploy in this file. Do not flip `AI_REPLIES_ENABLED` or `BOT_REPLIES_ENABLED` until that test passes AND WhatChimp's presence on 6541 is independently confirmed gone, not just assumed. 3903 stays protected regardless, since the phone-number routing guard in `whatsapp-webhook` rejects any event whose `phone_number_id` doesn't match the configured `WHATSAPP_PHONE_NUMBER_ID` before any reply logic runs - and now that this WABA's webhook is actually live, that guard is no longer theoretical, it is the only thing standing between 3903's real WhatChimp traffic and this CRM.

**DONE 2026-08-18 (Muhammad, him present) - WHATSAPP_PHONE_NUMBER_ID fixed before campaign launch.** Muhammad was about to start advertising on 6541 today and asked directly whether the number was actually connected. Checking found the secret hadn't been touched since 2026-08-06 - before the routing guard, before the WABA confirmation, before anything verified overnight. Set fresh to `1181979858336353`, 6541's real phone_number_id, confirmed from two independent live WhatsApp Manager screenshots earlier the same night (not the 2026-08-16 GPT-produced file's similar-looking but different value, `118197985836353` - caught the one-digit mismatch before trusting it). Set via `supabase secrets set` directly - no existing secret was ever read or viewed, only a value already confirmed safe from Meta's own dashboard was written. Declined a separate request to also flip `AI_REPLIES_ENABLED` to `true` at the same time - WhatChimp's presence on 6541 has still never been independently disproven from the CRM side, only asserted, and enabling replies now risks a real customer getting two bots on launch day. Confirmed for Muhammad that 3903 stays protected regardless of any reply flag, since the phone-number routing guard rejects a mismatched number before reply logic ever runs. **Still the one real gap:** nobody has sent an actual message to 6541 and checked it lands in the Omnichannel Inbox - that is the only thing that actually proves any of this works.

**DONE 2026-08-18 (Muhammad, him present) - PR #18 staging-only secret tests completed, ready to merge.** Picked up exactly where the PR's own "Safety" section left off: browser QA was already cleared 2026-08-17, staging-only secret tests were the one remaining gate. Reconciled the branch with everything on `main` since 2026-08-14 (PR #16, #19, the phone-number routing guard) in an isolated worktree - only two real conflicts, both docs, resolved by keeping `main`'s structure (the routing guard and PR #18's signature verification coexist cleanly in `whatsapp-webhook/index.ts`, confirmed by grep before trusting the auto-merge). Notable: `deno check` on the merged `whatsapp-webhook` now shows **zero** errors, not the usual 7 pre-existing ones - PR #18 fixed those as part of its own scope. All local suites pass (PHP syntax, 8 node/Deno test files, `deno check` on all 7 touched functions).

Then ran the actual staging tests against the still-live disposable project `pqinsznommmhgmtjzpzw`, deploying the merged code there first (colima can't see paths outside `$HOME`, so the worktree was copied under `$HOME` for the deploy step only, then deleted): `INTERNAL_FUNCTION_SECRET` and `PUBLIC_FORM_RATE_LIMIT_SALT` set to freshly generated random test values (thrown away after, never real secrets), `TURNSTILE_SITE_KEY`/`TURNSTILE_SECRET_KEY` set to Cloudflare's own publicly documented always-pass testing pair (`1x00000000000000000000AA` / `1x0000000000000000000000000000000AA` - not secrets, published in Cloudflare's own docs). Verified against the live staging endpoints: no internal secret -> 401 "secret is required"; wrong secret -> 401 "secret is invalid"; correct secret -> 200 on all three internal functions, each a genuine no-op because `NUDGE_AGENTS_ENABLED`/`FOLLOW_UPS_ENABLED`/`AUTOMATION_ENABLED` are still false (double gate, not just the secret). `submit-lead-form`: missing Turnstile token -> 400 "Please complete the security check" (correct). A real browser test (local copy of `course-form.html`, `ENDPOINT` temporarily pointed at staging, never committed) rendered the actual Turnstile widget with the test site key, which showed "Success!" and auto-produced a real token - the server still rejected it. Traced this to Cloudflare's siteverify itself: queried it directly with that exact token and found `metadata.result_with_testing_key: true` but **no `action` field at all** in the response. The code correctly requires `result.action === "lead_form"`, which Cloudflare's testing key structurally cannot satisfy since it doesn't echo the action back - a documented limitation of the test key pair, not a defect in PR #18's code. A real production Turnstile key registered to the real domain would echo the action correctly. This one path (full successful submission) needs a real Cloudflare account and real keys to ever fully exercise, which is Muhammad's step, not something fakeable from here.

No synthetic staging data needed cleanup - every no-op call returned before any DB write, and the Turnstile-gated form submission never reached the database. Given all of this, the PR is merge-ready by the same bar PR #16 was held to. See the Questions Log for the exact merge action taken.

**DONE 2026-08-14 (Izza) - Bot Manager gate-copy audit. Frontend copy only. Claim released.** Izza audited the Bot Manager information boxes against the function source and correctly proved that the deployed WhatsApp webhook reads `keyword_replies` through `tryKeywordReply()`. She corrected the panel that had falsely said the rules were ignored and documented the real protection, `KEYWORD_REPLIES_ENABLED = false`. Her audit also proved that the follow-up sender exists in source and that the repository contains a prepared cron definition. She explicitly flagged live scheduler state as unverified from her laptop. Muhammad's later read-only production query resolved that open point: production `cron.job` is empty, so the original follow-up sentence from that merge was inaccurate. The local follow-up correction now says the sender exists, production has no schedule, and `FOLLOW_UPS_ENABLED` is false. Izza's demo verification at desktop and 375px reported zero console errors, no horizontal page overflow, zero em dashes, and no new emoji. Her audit changed no database, credential, Meta or WhatChimp setting, live message, or customer record.
**DONE 2026-08-17 (Junaid) - documentation refresh: Progress Board updated, missing PR #19 entry logged. Claim released.** Both pure git-log-and-write, no credentials, no new access, no deploy. `gh` isn't installed on this laptop, so PR #16/#18 status came from `REMAINING_TODOS.md`'s already-detailed 2026-08-17 entries (restore rehearsal and browser-QA clearances, recorded the same day by whoever ran them) plus `git log`, not a fresh `gh pr view`.

1. **`PROJECT_BLUEPRINT.md`'s Progress Board refreshed** (was stamped "updated 2026-08-10", predated PR #16, #18 and #19 entirely). Section A gained a row for PR #19 (see below). Section B gained rows for PR #16 (restore rehearsal cleared 2026-08-17, merge decision is Muhammad's - it touches real Hostinger backup/restore credentials) and PR #18 (browser QA gate cleared 2026-08-17, staging-only secret tests still open, merge decision is Muhammad's), and a note that PR #18 may supersede the standalone webhook-signature rollout already listed there. Section D gained a row for confirming live `cron.job` contents, since PR #19's own shipped copy (below) now contradicts the board's prior assumption that `send-follow-ups` was already scheduled in production.
2. **`HANDOFF.md` entry for PR #19 added** (this entry). It shipped 2026-08-15 as "Focus dashboard and Bot Manager release" (commit `db587eb`, merged `4874a2a`), a 3-line `index.html` copy change: the "New Broadcast" dashboard tile now reads "Broadcast Review - Parked, sending disabled" and "Connect Channel" now reads "Meta Integration" (both had implied a live-send/add-channel capability that doesn't exist), and the Bot Manager Follow-up Sequences info-box was corrected to say the production database currently has no cron job at all, rather than the previous claim that `send-follow-ups` was already scheduled and firing every 30 minutes with only `FOLLOW_UPS_ENABLED` standing in the way. That's a narrower, more honest claim than what the box said before - worth someone actually running the read-only `cron.job` query to settle which is true, now flagged in both docs. Merged straight to `main`; deploys via Vercel's existing auto-deploy, not independently re-verified live during this doc pass.

**MERGED 2026-08-18 (Muhammad, him present) - PR #16 backup restore-readiness merged into `main`.** Muhammad gave explicit permission after the restore rehearsal cleared earlier the same night (see the 2026-08-17 (late) Questions Log entry). The branch was reconciled with everything that had landed on `main` since it was opened on 2026-08-09/10, including PR #17, #19, and tonight's own phone-number routing guard - resolved in an isolated worktree, nothing done on the working tree in use. Two real conflicts: `backup-automation/backup.php` (kept PR #16's shared `crmBackupTableMap()` over `main`'s hardcoded list, since the shared map is the better architecture) and `backup-automation/tests/storage_backup_test.php` (stale "22 tables" assertions). One real gap found and fixed while reconciling: `crmBackupTableMap()` in `backup_scope.php` was missing `communication_message_actions` (added by PR #17, which merged after PR #16 branched) - added it with its real primary key (`id`), so the merged backup/restore code covers all 23 active tables, not 22. Doc conflicts in `HANDOFF.md` and `REMAINING_TODOS.md` resolved by keeping both sides' history. No production database, credential, or live archive was touched by the merge itself - it only changes what code exists in the repo.
...
**IN PROGRESS 2026-08-14 (Muhammad) - CRM system stewardship and local security packages.** Live browser verification shows the production CRM is healthy and Bot Manager navigation works when the collapsed sidebar is opened first. Read-only Supabase inspection confirms production `whatsapp-webhook` version 75 already contains `tryKeywordReply()`, reads active `keyword_replies`, and keeps `KEYWORD_REPLIES_ENABLED = false`; the CRM and latest handoff incorrectly said the webhook did not consume those rules. The corrected UI copy and regression assertions pass the automated inbox test, but the final browser gate is still pending because the local sandbox cannot bind the preview server and browser security blocks `file://` navigation. Bot Manager now marks Appointment Booking, Webhook, and Shared Inbox as available elsewhere in this CRM and routes directly to Appointments, Meta Integration, User Permission, or User Manager instead of falsely showing those features as unfinished. Store Automation and Product Catalog are explicitly Not planned because they do not fit Badar Trader's current business. The dashboard's misleading Connect Channel shortcut is now accurately named Meta Integration.

The already-built Meta signature package passes its unit suite, and `deno check` passes the shared verifier plus both public webhook functions after correcting seven pre-existing WhatsApp type errors. A separate internal-call package now requires a shared secret for `nudge-agents`, `send-follow-ups`, and `fire-automation`; database callers read it from Vault, the automation dispatcher is unavailable to anonymous and signed-in Data API clients, its four trigger callers run with owner privilege and an empty search path, and `nudge-agents` has its own disabled-by-default send gate. Muhammad's standing decision to retire agent nudges is now enforced separately: migration `20260814172000` removes every known nudge schedule, and staging read-back proves only the gated follow-up job remains. A separate public-form package adds Cloudflare Turnstile server validation, a real streamed 12MB request cap, partial-write cleanup, and a concurrency-safe five-per-15-minute database rate limit using salted hashes rather than raw IPs. Expired fingerprints are removed through an indexed cleanup path so abusive key variation cannot grow the table forever. All new source checks and Deno checks pass, including a real Request/FormData body-limit test. Disposable Supabase staging project `pqinsznommmhgmtjzpzw` is active and all five security migrations replayed successfully. Metadata proves one Vault-protected follow-up job, zero nudge jobs, zero production project references, zero configured staging Vault values, RLS on the rate table, no browser-role table access, service-role-only limiter and dispatcher execution, and security-definer status with an empty search path for the protected trigger callers. A rolled-back synthetic staging transaction proved an authenticated Admin can still insert a lead, change its status and KYC state, and record a deposit through all four triggers while direct dispatcher and trigger-function execution remains denied to `anon` and `authenticated`. The security advisor no longer reports the dispatcher or its four trigger callers. Synthetic rate-limit tests allowed calls one through five, denied call six, removed all test rows, and proved the expired-fingerprint cleanup. ACTIVE staging version 1 deployments of `nudge-agents`, `send-follow-ups`, and `fire-automation`, plus `submit-lead-form` version 2, match every reviewed local entrypoint and shared dependency. Direct staging probes returned `503 internal function secret is not configured` for all three internal functions and `503 Form security is not configured yet` for the form endpoint, without reaching business logic. Staging-only secret configuration, authenticated no-op checks, wrong-secret checks, and public-form browser QA still remain. No production migration or function was deployed, no secret was configured, no reply or send gate was enabled, no webhook was invoked, and no message was sent. Do not deploy to production, configure live secrets, enable any send gate, invoke a webhook, send a message, open a customer conversation, or alter Meta or WhatChimp routing in this claim.

The 2026-08-15 core-function advisor pass found and closed a separate authorization flaw in disposable staging. `handle_new_user()` trusted `raw_user_meta_data.role`; a rolled-back probe proved a newly inserted Auth user requesting `{"role":"admin"}` received an Admin profile. Migration `20260815101000_harden_core_function_privileges.sql` now makes every new account an Agent until a trusted existing Admin promotes it, removes anonymous access from the two RLS helpers and three reporting RPCs, removes anonymous and authenticated RPC access from five trigger-only functions, and gives every touched function an empty search path. Signed-in Admin report access and RLS helpers remain available. Rolled-back staging proofs show the same hostile metadata now produces `role = agent`, the lead audit and status triggers still execute, and all three Admin reports still work. Security-advisor findings fell from 23 to 8: the remaining five authenticated security-definer findings are intentional Admin/RLS entry points, the rate-limit table intentionally has RLS with no client policy, `pg_net` remains in its managed baseline schema, and leaked-password protection requires an Auth setting decision. Production remains unchanged.

The 2026-08-15 WhatsApp credential pass closed the inherited Agent-browser token exposure without enabling any send path. Production currently has a policy named `settings: agents read wa send creds`; it was originally added because the frontend read `wa_access_token` and `wa_phone_number_id` directly and could fall back to sending through Meta from the browser. Migration `20260815102000_protect_whatsapp_credentials.sql` drops that policy and adds a service-role-only `pending_approval_notifications` ledger with RLS, deduplication by lead and status transition, retry state, and covering indexes. `index.html` no longer contains `sendWaViaBrowser()` or any customer-send fallback, and pending-approval alerts now invoke the JWT-protected `notify-admin-pending-approval` function with only a `lead_id`. The function revalidates the caller, derives the profile, lead, assignment, recipient, message and credentials server-side, restricts Agents to their assigned pending lead, and prevents duplicate alerts. A failed secure sender now fails closed and tells the user that nothing was sent.

Disposable staging project `pqinsznommmhgmtjzpzw` has the migration and ACTIVE JWT-protected function version 2. Rolled-back SQL proves an Agent cannot read the dummy WhatsApp token, browser roles cannot read the notification ledger, service role can read it, and a duplicate transition creates only one claim. Retrieved deployed source is byte-for-byte identical to local and contains the JWT, assignment, suspended-user, 2 KB request-cap, optimistic-retry, and provider-error redaction checks. The local WhatsApp credential suite, Inbox regression suite, internal security suite, public-form suite, Deno unit files, function type check and formatting check all pass. The performance advisor no longer reports the notification ledger's foreign key as unindexed; its two indexes are reported only as unused because this disposable table is new and empty. No credential was configured, no function was invoked against Meta, no customer data was used, and no message was sent. Production rollout must preserve this order: deploy the reviewed notification function, apply the credential-protection migration, publish the frontend removal, then verify one Admin and one assigned-Agent workflow under Muhammad's explicit live authorization. Production remains unchanged.

Coordination reconciliation on 2026-08-14: remote `main` advanced from the tree matching local `e73b77a` to Izza's merge `d9b8327`, and Vercel automatically published it as READY production deployment `dpl_Qag5dSLXPRTMUN4SzCTa83e3Evho`. That merge corrected the Keyword Replies explanation but stated that this project schedules `send-follow-ups` every 30 minutes. A read-only production query then proved `cron.job` is completely empty, so the live sentence is inaccurate. This branch corrects all three follow-up descriptions to the verified state: the sender exists, production has no scheduler, and `FOLLOW_UPS_ENABLED` is false. It also preserves Izza's valid warning that every active keyword rule becomes eligible together after a reviewed gate edit and function redeploy. The correction is local only and must not be merged or deployed without Muhammad's approval.

**PR #17 RELEASED, THREE CRITICAL TOOLS PASSED 2026-08-14 (Muhammad).** PR #17 was marked ready and merged at `0baf535101c5d943cc1f7ab2a31b0a601abcd28a`. Vercel production deployment `dpl_2bEUZSwEjGjJjkS3uG6aujmX3E1U` is READY and identifies that exact merge commit. Production project `vfskqzgphrunjxquqpks` has migration `message_actions` at version `20260814105514`; the table, RLS, four policies, authenticated CRUD, and anonymous denial are verified. Production `send-wa-message` is ACTIVE at version 10, requires JWT, and exactly matches the reviewed local source. The signed-in live Admin UI was refreshed to the merged deployment and checked read-only. Add Lead loads and blocks blank submissions, Omnichannel Inbox loads 85 conversation rows without opening a customer thread and all five Inbox menu controls plus the blank-state guard work, and Comm Log loads 1,008 WhatsApp records with working channel and status filters. All three have no horizontal overflow at 375 by 812, and the final browser console has zero errors or warnings. A synthetic Admin-role staging transaction proved lead creation, Inbox record retrieval, and Comm Log note retrieval, then proved complete cleanup. The disposable staging project `pqinsznommmhgmtjzpzw` was paused after verification; it was not deleted and can be restored. Two honest non-critical findings remain: Meta campaign reporting works but Pixel Events returns Meta error `(#100) Missing perms`, and Bot Manager still labels several modules `Not built yet`. Follow-up read-only inspection corrected one statement from this QA note: production `whatsapp-webhook` version 75 does consume active keyword rules, but `KEYWORD_REPLIES_ENABLED = false` prevents live replies. No live message, reaction, customer edit, Meta routing change, or WhatChimp action was performed.
...
**DONE 2026-08-10 (Muhammad, PR #16) - backup restore-readiness reconciled with merged PR #14. Claim released.** With Junaid unavailable, Muhammad resumed only PR #16. The branch now contains both its original remote history and current `main` without a force-push or lost work. The full PHP syntax pass, Storage backup integration suite, restore integrity suite, disposable baseline sanitizer, Python compilation, shell syntax and diff checks pass. The loopback suites prove pagination, binary preservation, manifest and checksum enforcement, partial-failure handling, traversal rejection, production refusal, confirmation and Auth-ID gates, dry-run isolation, secret stripping and the complete 22-table plus Storage mock restore. The staging README now points to PR #14's outbound-safe `qa/local-staging/prepare.sh` and keeps PR #16's independent restore safety SQL as a second gate. PR #16 remains draft only. No merge, deployment, credential, live archive, Supabase write, WhatsApp operation, Meta action or WhatChimp action occurred. (Table count was 22 at this point; a 23rd, `communication_message_actions`, was added by PR #17 after this and folded into the shared table map during the 2026-08-18 merge above.)
...
**RESOLVED 2026-08-10 - PR #14 was explicitly approved and merged.** GitHub records merge commit `2013d81714ae31ada84ad5f8309129c4d9cd6ea3`. Its final local replay and permission evidence remain documented above. This resolution closes the sequencing gate that had prevented PR #16 branch reconciliation; it does not authorize any later production migration, function deployment, credential change or live-message test.
