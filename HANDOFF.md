...
## 2026-08-08 - Lead lifecycle + payroll plan, Settings hub, Bot Manager visual pass, collapsible nav, four layout bugs

Muhammad's brief, in one message: payroll's real truth is Converted leads, so tighten the hierarchy behind every status - but **Badar will not approve each lead an agent drops into Converted**, so find another rock-solid plan and go look at what competitors do. Plus a Settings section in the side panel (also competitor-researched), plus the Bot Manager being "dull and boring" (Send button too small, field names flat, no Create New Campaign on the dashboard). Mid-session he added three more: go into **every** section on the main dashboard, not just Bot Manager; a new hardened rule that UI is never reported as done unverified; and consolidate the left nav into collapsible headings with a dropdown arrow.

**1. `docs/LEAD_LIFECYCLE_AND_PAYROLL.md` - the plan, researched, not invented.** The competitors that matter here are not the WhatsApp CRMs (WhatChimp, Wati, AiSensy, Interakt, Respond.io) - they have no opinion on this - but the **forex broker CRMs**: FXBO, Leverate, B2Core, UpTrader, AltimaCRM, Syntellicore. The finding that answers Badar directly: **in those systems a conversion is not a status an agent picks, it is an event the system observes.** The commission trigger is the FTD, confirmed by back-office deposit data. So:
- All seven statuses now have a written definition, binary entry evidence, who may set it, and legal transitions.
- **Converted comes out of the dropdown entirely.** An agent's end of the line is "Deposit Reported" - a claim. The system decides Converted. That one change removes the thing Badar objects to rather than managing it.
- Verification runs in tiers: auto-verify on seven machine-checkable rules (screenshot present, account ref format, amount over the floor, **account ref not already used by another lead**, phone matches the thread, lead not created seconds before the claim, claiming agent is the assigned agent); anything failing goes to an **exceptions queue** that any admin can clear, with four-eyes (not the agent who claimed it); plus a random spot-check sample so auto-verify stays honest.
- Payroll counts `verified_at`, never `converted_at` and never `status`, with a clawback window and run snapshots.
- **Stated honestly in the doc:** the real source of truth is the Exness/XM IB portal, and that reconciliation is NOT built - it needs an export from Badar's own broker accounts. Everything above is evidence-based and could in principle be fooled by a convincing screenshot.
- **Badar has to supply five numbers** (minimum deposit, commission %, clawback days, spot-check %, auto-verify on or off) and one policy (who besides him may clear an exception). All five already have a field waiting in Settings.

**2. Settings hub built.** There *was* a SETTINGS heading in the sidebar, but 20 items down, below the fold - which is why it read as missing. Now one Settings entry opening a hub of grouped cards (Workspace, Channels, Pipeline and money, Business rules, Data and help), matching how every CRM checked does it. Two real forms save to the existing key/value `settings` table, so **no migration was needed**; they carry the payroll thresholds above, with input validation that refuses a non-numeric or out-of-range value rather than storing a typo that later decides who gets paid. Labelled honestly: nothing reads those keys yet.

**3. Bot Manager visual pass, and it found a genuine bug rather than only a dull one.** Gold field labels (the brand colour - they were grey `#64748b`, reading as disabled), gold rules under form headings, and primary buttons rebuilt gold at 13px/26px instead of the flat sky blue. The Conversations **Send** button was `.btn-sm` - 5px of vertical padding on the button an agent presses more than any other in the product - now 10px/22px and bold. "Create New Campaign" added to the dashboard tiles and to the AI Knowledge Base panel, landing straight on an empty form (it was four clicks deep).

**4. Four layout bugs, every one found by screenshotting rather than by reading code.**
- **Bot Manager's whole right edge was clipped.** `.bm-layout` used `grid-template-columns: 300px 1fr`, and a `1fr` track refuses to shrink below its content's min-content width - so the Bot Settings button, the Webhook Workflows sub-tab and the Edit/Delete buttons on every table row were pushed off-screen and unreachable. `minmax(0, 1fr)` plus scroll containers on the sub-tab rows. Measured before and after: content area overflowed by 91px, now 0.
- **Table action columns disappeared** behind long reply/prompt text. The text column now truncates; the buttons always fit.
- **Four tabs overflowed on a phone** (Appointments 116px, Broadcast Signal 219px, Subscribers 198px, AI Signals 5px). Cause: hardcoded `grid-template-columns:repeat(4,1fr)` stat rows instead of the responsive `.stat-grid` class this file already has - same bug class as the Bot Manager one. Fixed by reusing the existing class, not by adding an override.
- **Pipeline Overview rendered as a large empty box** whenever Chart.js failed to load, because the render function returned early and drew nothing. It now shows the real status counts (no library needed) and says why the chart is missing. This is also the answer to Muhammad's older question about the missing donut - in production the CDN loads and it draws; the blank box only appears when that request fails.

**5. Sidebar consolidated into 5 collapsible groups** (was 7 headings and 24 loose items): MAIN, ENGAGE, SIGNALS, FINANCE, SETTINGS. The old BOT MANAGER heading held one item and folded into ENGAGE; SYSTEM (User Manager, User Permission) folded into SETTINGS where every other CRM puts them. Each heading is a button with a caret, collapsed groups show a count of what is hidden, state is remembered per browser in localStorage, and **the group holding the active tab is always force-opened** so nobody lands on a screen whose nav item is hidden.

**Verification, under the new rule.** Every one of the 22 admin sections was walked in demo mode at 1440px and 375px, in both themes, with screenshots looked at rather than just console output: **0 horizontal overflow, 0 console errors, 22/22 sections**. The nav has its own 12 behaviour checks (collapse, expand, caret rotation, hidden count, persistence across reload, force-open on navigation, and a corrupt-localStorage case that must not break the nav) - all passing. Nothing was deployed and no live data was touched.

**New standing rule added to `CLAUDE.md`:** UI is never reported as done unverified either - a clean console says nothing about whether a screen is usable, so screenshot the rendered section and look at it, checking clipping, sideways scroll, tap targets, both themes, 375px, and whether empty states explain themselves.

---

## 2026-08-08 - Schema drift closed, converted_at stamping fixed, two real bugs found (remote session, nothing deployed)

**Context worth stating first: this session ran remotely, not on Muhammad's, Junaid's or Izza's laptop.** So every remaining item at the top of this file was out of reach by rule or by circumstance - the Box 3 `whatsapp-webhook` deploy (Muhammad's laptop, him present), `config.php` on Hostinger (only Muhammad), and the Meta App Dashboard webhook-routing question (no Claude session may open it). Rather than stall on those, this picked up the **"TO BUILD - safe code work, no live send"** list in `PROJECT_BLUEPRINT.md` and cleared two items off it. **Nothing was deployed, no live system was touched, and no migration was applied.**

**1. Schema drift closed** - the oldest item on that list. `communication_logs` is written by three Edge Functions (`conversion-hook`, `submit-lead-form`, `send-wa-message`) and three places in `index.html`, read by the whole Comm Log tab, and had **no `CREATE TABLE` anywhere in this repo**. Six `leads` columns were in the same state: `converted_at`, `verified`, `deposit_platform`, `deposit_amount`, `deposit_account_ref`, and `bot_stage_history` (that last one had a migration but was never written back into `schema.sql`). Now defined in `schema.sql` as Phase 29 and in migration `20260808000000_schema_drift_backfill.sql`.

**Nothing needs applying to live.** The migration is deliberately a no-op there: every statement is guarded, there is no `DROP` in the file, and the RLS block only fires where the table has no policies at all. All of these objects already exist live. The value is that a rebuild from this repo now produces a working database instead of a broken one.

**This was verified by actually building the database, not by reading the SQL back.** A real Postgres 16 was stood up locally with a shim for the parts Supabase provides (`auth.uid()`, roles, pg_cron, pg_net, storage), and both versions of `schema.sql` were applied to it:
- **Before:** the four write patterns deployed code performs every day were run against a database rebuilt from the old `schema.sql`. `conversion-hook`'s own deposit UPDATE **failed on the missing `verified` column** - so the drift was not theoretical, a rebuild really was broken.
- **After:** the same four writes all succeed.
- The migration is **idempotent** (clean second run, only "already exists, skipping" notices), and against a **live-shaped database seeded with a different pre-existing policy** it left that policy completely untouched - which is the case that actually matters, since applying a reconstruction over live RLS could silently widen who can read customer notes.
- The reconstructed policies were then exercised for real: an admin sees every row, an agent sees only rows on leads assigned to them, an agent inserting on someone else's lead is refused, forging `created_by` as a colleague is refused, a bad `type` value is refused by the CHECK, and deleting a lead cascades its logs away (3 rows to 0).

**The honest limit:** these definitions are **reconstructed from every read and write site in the code, not dumped from the live catalog** - no session here has database credentials. Column names, nullability and the `type` CHECK are certain; exact live types and the live RLS policy text are not. Whoever next has DB access should diff them. Same caveat `20260806010000_profiles_phone` already carries.

**2. The `converted_at` stamping hole is fixed** - also off that list. `approveConversion()` and `conversion-hook` have always stamped `leads.converted_at`, but the status dropdown never did, so any lead converted by hand through `saveLeadDetail` (admin) or `agentSaveStatus` (agent) was left unstamped forever. A shared `convertedStamp()` helper now stamps on the transition into converted, and **only if the lead is not already stamped**, so re-saving an already-converted lead never moves its conversion date. Both the demo and live branches were changed, deliberately together - a demo/live mismatch is the exact thing that has hidden bugs in this project repeatedly.

**Verified in a browser in demo mode**, driving the real UI (clicking the All Leads nav item, clicking the lead row, setting the real `#det-status` select, clicking the real Save Changes / Update Status button) - **22 of 23 checks pass**: admin and agent paths both stamp, re-saving does not move the stamp, a converted -> contacted -> converted round trip keeps the first stamp, a non-converted status change stamps nothing, and seven helper boundary cases including a null/missing lead object. The 23rd check is console errors, and that one is **this remote container, not the change**: the sandbox's proxy blocks the CDN, so `window.supabase` never loads. Confirmed by running the identical capture against the unmodified `index.html` from `HEAD` - **4 errors before, the same 4 after**. Nothing about this change adds a console error. Zero requests to any Supabase host from demo mode.

Nothing reads `converted_at` yet, so this changes no screen today. It is worth doing now precisely because it is a data-quality hole: no time-to-conversion reporting can ever be built over a column that is only populated for some conversions, and every month it stays broken is a month of rows that cannot be backfilled.

**3. Real bug found on the way: XM deposits have been recorded as "other".** `join.html`'s broker dropdown offers Exness / XM / Other, but `conversion-hook`'s `PLATFORMS` whitelist was `["exness","dooprime","course_only","other"]` - **no `xm`** - and anything not on that list is coerced to `"other"`. So every XM depositor is stored as `deposit_platform = 'other'`, which is what the leads table renders as an "OTHER" badge and what the Dashboard's revenue "By platform" breakdown lumps in with genuinely-unknown brokers. Fixed by adding `"xm"` - one array entry, purely additive, nothing that worked before behaves differently. **Historical rows stay `other`** and would need a manual backfill if the split matters; that is a decision for Muhammad, not something to do silently. **This needs a `conversion-hook` deploy from Muhammad's laptop** - logged in REMAINING_TODOS.

**4. Small gap closed: `notifications` was missing from the backup script's table list.** `backup-automation/backup.php` enumerates tables by hand on purpose (so a run's scope is always exactly what was reviewed), and that list stopped at `ai_agents` - the `notifications` table added to `schema.sql` on 08-07 was never added. Since its migration is parked, today the run just logs a skip for it; the point is that the day it is applied it is already covered rather than silently absent from every backup. **Re-verified the whole script end to end against a mock PostgREST server** the same way it was originally built: 19 tables OK, the two absent tables logged as skips without aborting the run, 2500 rows correctly paginated into the archive, and pruning across four consecutive runs kept exactly the configured retain count.

**One extra data point for whoever does the Box 3 deploy:** all nine Edge Functions, including Izza's untyped-checked `whatsapp-webhook` change, were **syntax-parsed clean with the TypeScript compiler**. That is not `deno check` - it does not resolve the `https://esm.sh` imports or type-check against them (no Deno here, and the proxy blocks installing it) - but it does rule out the unbalanced brace or malformed literal that a hand edit with no tooling is most likely to leave behind. The `deno check` before deploying is still worth running.
