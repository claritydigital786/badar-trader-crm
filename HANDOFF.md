...
**Next on "continue 2": Type 3 (tool-using action agent).** Reuses `shared/llm.py`; adds a reason-act loop over a small set of tools (start with 2-3 safe read-only ones, e.g. a calculator and a local file/doc lookup), a step cap so it cannot loop forever, structured logging of every tool call, and - non-negotiable for this repo - a human-approval gate before any tool that could send, spend, or make an irreversible change. Keep every tool sandboxed; no live CRM/WhatsApp/Meta/WhatChimp access.
