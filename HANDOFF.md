...
**Next on "continue 2": Type 4 (multi-agent system).** An orchestrator that breaks a task into parts and delegates each to a specialised worker agent (each worker essentially a focused Type 3 agent), then combines the results. Roadmap's stated pitfalls to avoid: reaching for multi-agent when one good Type 3 agent would do, fuzzy roles that overlap, and no overall step/budget cap across the whole system (on top of each worker's own cap). Reuse `type3_agent/` workers rather than rebuilding tool-calling from scratch.
