...
**DONE, CORRECTED AFTER MUHAMMAD'S PR REVIEW 2026-08-10 (Junaid, Ayesha laptop) - assigned-lead RLS and safe local staging. Claim released.** Added local corrective migration `20260810020000_restrict_agents_to_assigned_leads.sql` and synchronized the authoritative schema so Admin retains full access while each active Agent can access only the Agent's assigned leads, related records, private lead files, and own appointments. Muhammad's PR comments then found two further gaps: `prepare.sh` copied production cron and `pg_net` automation callbacks into local staging, and the parked notifications table still entered through both `schema.sql` and its migration. The final preparer now sanitizes the baseline before Docker starts, replaces automation with a local no-op, removes all three production cron schedules, excludes Phase 28 and its migration, and fails if any prepared SQL contains the production project reference, `cron.schedule`, `net.http_post`, or the parked notifications table. A fresh disposable rebuild applied 32 applicable steps, schema lint returned no errors, and the expanded cross-Agent matrix passed 75/75 checks including own and cross-Agent appointment reads, updates, and inserts. Browser QA confirmed all 20 active Admin and 6 Agent modules, assigned-lead visibility, 375-pixel mobile containment, no missing-table console error with parked Notifications absent, and zero console errors or warnings. Evidence is in `qa/local-staging/evidence/QA_REPORT.md`. This package used fake local data only and did not query or change production, deploy anything, use credentials, touch live accounts, or send a message. PR #14 remains open, mergeable, and unmerged. The next package waits for Muhammad's review and uses a new branch.
