...
- [x] Type 2 - RAG agent (DONE 2026-08-06, verified: 6 self-tests pass, scripted CLI retrieves the right source and refuses out-of-doc questions)
- [x] Type 3 - tool-using action agent (DONE 2026-08-06, verified: 15 self-tests pass, scripted CLI run exercised all 3 tools including both approve and deny on the dangerous one)
- [x] Type 4 - multi-agent system (DONE 2026-08-06, verified: 7 self-tests pass, scripted CLI run showed a compound task delegating to two workers and a single-domain task delegating to one - all four roadmap types now built)
...
**Type 2 build (2026-08-06, Muhammad's laptop):** `ai-agents/type2_rag/` - `chunking.py` (paragraph-aware, overlapping), `vector_store.py` (in-memory cosine search behind an `add`/`search` shape you can swap for Chroma/pgvector), `rag_agent.py` (ingest -> retrieve -> grounded prompt with citations, plus a similarity-threshold "I don't know" that fires WITHOUT calling the model, so the refusal is a hard guarantee), a `sample_docs/` demo set, and a CLI. `shared/embeddings.py` added the provider seam: `StubEmbedder` (deterministic local bag-of-words, offline, zero cost - the default) plus `OpenAIEmbedder`, switched by `EMBED_PROVIDER`. **Embedding decision:** default to the offline stub so RAG runs and tests with no key and no surprise API spend; real semantic embeddings drop in via `EMBED_PROVIDER=openai`. Verified: 6 Type 2 self-tests pass, Type 1's 4 still pass, all modules byte-compile, and a scripted CLI run ranked the correct source first for three in-doc questions and refused an out-of-doc one. Stub LLM echoes the grounded prompt rather than synthesising prose (expected; real key lights that up) - the retrieval half is fully real.

**Type 3 build (2026-08-06, Muhammad's laptop):** `ai-agents/type3_agent/` - `tools.py` (three tools: `calculator`, evaluated with an AST whitelist rather than `eval()` so it cannot run arbitrary code; `doc_lookup`, which reuses Type 2's chunker/stub-embedder/vector-store to retrieve from `type2_rag/sample_docs/` but stops after retrieval instead of calling a model - composition of one type inside another; `send_notification`, marked `dangerous=True` - it does not reach any real system, it only appends one line to a local `outbox.log`, standing in for "a message went out" so the approval gate has something concrete to guard), `stub_planner.py` (`StubReactClient`, a deterministic keyword-based planner used only in stub mode so the full reason-act loop is provable offline with no key - swapping in a real Claude/OpenAI key needs no protocol change), and `agent.py` (`ActionAgent`: a plain-text `TOOL: name | input` / `FINAL: answer` protocol over the same `LLMClient.stream()` seam Type 1/2 already use, a step cap default 6, a human-approval callback that pauses the loop for any dangerous tool and feeds a denial back as a recoverable observation rather than crashing, and a full per-step trace). `tests/test_type3.py` added (15 stdlib self-tests, no pytest): calculator correctness and code-injection rejection, doc_lookup retrieval and refusal, stub planner routing for all three tools, full-loop resolution of a math task and a doc task, dangerous-tool denied (outbox untouched) and approved (outbox written) paths, the step cap actually stopping an agent that never says FINAL, an unknown tool name recovering instead of crashing, and malformed model output being treated as final instead of retried forever. Verified: all 15 pass, Type 1's 4 and Type 2's 6 still pass, every module byte-compiles clean, and a scripted CLI run (`python3 -m type3_agent.cli`) exercised all three tools end to end including both a real approval and a real denial of `send_notification`. `outbox.log` is gitignored so no runtime artifact gets committed. **Unverified deliberately:** the real Claude/OpenAI reasoning path was not run - no key on this laptop - only syntax-checked; first real-provider run is Muhammad dropping a key into `.env`, same as Type 1/2.

**Type 4 build (2026-08-06, Muhammad's laptop) - all four roadmap types now built.** `ai-agents/type4_orchestrator/` - `workers.py` (three workers, each exactly a Type 3 `ActionAgent` given one tool only: `math_worker`/calculator, `docs_worker`/doc_lookup, `ops_worker`/send_notification - narrow on purpose, since the roadmap's own warning for this type is fuzzy overlapping roles; each worker inherits Type 3's approval gate and step cap automatically, no new guardrail code needed), `orchestrator.py` (`Orchestrator`: a `DELEGATE: worker | subtask` / `FINAL: answer` protocol one level above Type 3's `TOOL`/`FINAL`, reusing the same `LLMClient.stream()` seam; a `max_delegations` cap default 5 - a second, separate budget from each worker's own `max_steps`, because a runaway orchestrator and a runaway worker are different failure modes and one cap does not catch both; a full trace including each worker's own sub-trace), and `stub_planner.py` (`StubOrchestratorClient`: deterministic delegation across the same three domains Type 3's stub covers, delegating to whichever matching worker has not yet reported a result - so a compound task like "what is 12 * 4, and what is your refund policy?" genuinely exercises two workers in one run, and a single-domain task delegates to exactly one, demonstrating the roadmap's own "don't reach for multi-agent when one agent would do" restraint rather than ignoring it). `tests/test_type4.py` added (7 stdlib self-tests): single-domain task uses one worker, compound task uses two and combines both results, an unknown worker name recovers instead of crashing, the delegation cap stops a client that never says FINAL, malformed orchestrator output is treated as final, and the approval gate correctly flows through the orchestrator into a worker's dangerous tool call for both a denial (outbox untouched) and an approval (outbox written). Verified: all 7 pass, Types 1/2/3's self-tests (4+6+15) still pass, everything byte-compiles clean, and a scripted CLI run showed a real compound-task run delegating to math_worker then docs_worker and combining both, a single-domain task delegating to only math_worker, and a notify task correctly pausing for a real approval prompt through the orchestrator. `outbox.log` (written by `ops_worker` via the shared Type 3 tool) stays gitignored. **Unverified deliberately, same reason as every other type:** no real Claude/OpenAI key on this laptop, so real reasoning was never run - only syntax-checked.

**"continue 2" roadmap status: all four types are now built.** What's next is not more roadmap - it is a real decision from Muhammad about whether/how this graduates from a learning build into something connected to real data. See `ai-agents/README.md`'s "What's next" section: real reasoning needs only a key in `.env` (no code change); real tools (CRM data, a real send path, etc.) is a scope decision that needs explicit sign-off before anything here goes near live CRM/WhatsApp/Meta/WhatChimp data, per this track's hard safety rule below. Muhammad asked directly (2026-08-06) whether these agents are "capable of anything" beyond a chatbot and said he wants "a real AI agent which works as a complete resource" - that question is still open and is the natural next conversation on this track, not a new build task to start solo.
...
- Since all three machines share one GitHub identity, commits are told apart by the **local git `user.name`** set on each laptop, not by separate accounts - this laptop (Muhammad's) is set to `Muhammad`, Junaid's (including his backup "Ayesha" laptop - same person, see REMAINING_TODOS.md 2026-08-06 entry) is set to `Junaid`. Izza's laptop still needs `git config user.name "Izza"` run once his setup is unblocked - see the onboarding note below, still in progress as of today.
...
**Izza's laptop onboarding, started 2026-08-06, still in progress 2026-08-07:** last known state was stuck on the Xcode Command Line Tools install (`xcode-select --install` run, same error persisted, `softwareupdate --list` suggested as the next diagnostic step, not confirmed). Muhammad is checking current status and will send a screenshot. Once Xcode tools are confirmed working, the remaining steps mirror how Junaid's laptop was set up originally: clone this repo, set up GitHub auth if the laptop doesn't already have it (SSH key is what Junaid's needed, see REMAINING_TODOS.md's 2026-08-05 entries), `git config user.name "Izza"`, confirm `git fetch origin` shows `main` with no stray local-only commits, then read this file's "Active Work Claims" before picking up anything.
