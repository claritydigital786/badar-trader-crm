...
**STAGING PASSED, RELEASE CORRECTION LOCAL 2026-08-14 (Muhammad) - PR #17 staging.** Muhammad approved the $0 per month disposable project alternative. Supabase project `badar-crm-pr17-disposable-20260814` (`pqinsznommmhgmtjzpzw`) is active in `ap-northeast-1`, the same region as production. The sanitized schema removed production callbacks, cron schedules, hosted function URLs, the production project ref, and parked Notifications before application. The exact message-actions migration then applied from an absent-table state. Fake Admin, Agent A, Agent B, assigned leads, and fake communications proved assigned-only conversation access, per-user action privacy, allowed owner writes, and denial of ownership spoofing, cross-Agent writes, ownership-changing updates, and overlength reactions. The first advisor run found that legacy Supabase default privileges still granted `anon` table access even though RLS blocked rows. The migration now explicitly revokes `anon` and regrants authenticated CRUD, with a regression assertion. A clean final replay proves the table exists, RLS is enabled, all four policies exist, authenticated CRUD is true, anonymous SELECT and INSERT are false, and assigned-owner insert plus cross-Agent denial pass. `node tests/inbox-ui-test.mjs`, `git diff --check`, and `deno check` pass. Staging `send-wa-message` version 1 is ACTIVE, requires JWT, and is byte-identical to local source. The browser sandbox blocked the final HTTP invocation before any request left the machine, so no reaction or message was sent. Production remains unchanged. Next: commit and publish this correction to PR #17, review the updated PR head, then apply the final migration and Edge Function to production before merging the frontend.
