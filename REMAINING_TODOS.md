...
- [ ] **Inbound non-image media is silently dropped - real bug, found 2026-08-06 by the parity audit.** A customer sending a voice note, PDF, video, sticker, location or contact card produces **no `communications` row at all**, so the agent never sees that anything arrived. `extractUserInput()` in `whatsapp-webhook` handles only text and interactive replies; everything else logs "Skipping unsupported message" and continues. Minimum fix is to always write a row with a readable placeholder (for example `[voice note]`, `[document: receipt.pdf]`) so the conversation is honest; downloading and storing the media is a larger follow-on. This is a webhook change, so it needs a deploy window.
...
- [ ] ~~**WhatsApp feature-parity check (Muhammad, 2026-08-06):**~~ "Have you added all of the features which we could see on WhatsApp?" Go through the real WhatsApp interface feature by feature and confirm each one is present in the Conversations dashboard, or list exactly what is still missing. This is the parity verification for the WhatsApp Web skin built on 2026-08-05.
