...
## 3.9 A second client changes the brief: Shahbaz, and Instagram

Muhammad shared an Instagram DM screenshot from **Shahbaz**, a second forex client who
is also a social media influencer. Two separate things are visible in it, and both
matter to this product.

**(a) The lead in that thread is exactly our target, and it arrived on a channel we do
not support.** The message reads: "I want to learn trading, I am already doing forex but
yet not profitable, what should be the procedure. What is your fees and what will be the
discount as a follower of mam zaynab?" That is a qualified, high-intent, referral-attributed
lead - the best kind this business gets - and it landed in **Instagram DMs**, which this
CRM does not read. Bot Manager already shows an "Instagram 0" channel chip. For Shahbaz,
Instagram is not a nice-to-have channel, it is probably the main one.

**(b) The account is restricted and cannot reply.** The screenshot shows "You can't send
messages at this time. Certain actions may be limited due to our Community Standards."

**Be very clear with both clients about what software can and cannot do here.** A CRM
**cannot** send on a restricted account and must not try - the restriction is enforced by
Meta at the account level, and anything that looked like working around it would put the
account at far greater risk than a missed reply. Any vendor who implies otherwise is
selling something dangerous.

What a CRM **can** do, and what would have saved this lead:

1. **Capture the message anyway.** Instagram DMs can be ingested through the same Meta
   Graph/webhook stack the WhatsApp side already uses. Even with sending blocked, the lead,
   their question, and the referral attribution ("a follower of mam zaynab") would be
   captured as a real lead instead of sitting unread in an app.
2. **Alert a human immediately**, because a restricted account is exactly when a person
   needs to know a warm lead is waiting.
3. **Continue on a channel that is not restricted** - the lead has already asked a
   question, so replying from the business WhatsApp number, or sending them a link to it,
   is a legitimate route that the customer themselves opened.
4. **Show the restriction in the CRM** rather than letting sends fail silently. We already
   surface Meta send errors on the WhatsApp side; the same treatment applies.

**Product consequence:** Instagram DM as a real channel moves up the roadmap the moment
Shahbaz is a client, and this CRM is already architecturally close - same Meta app, same
webhook pattern, same inbox. It is the clearest case yet for the "omnichannel" in
Omnichannel Inbox being real rather than aspirational.

**Second consequence, worth deciding early:** two clients means **multi-tenancy**. Today
every table is single-business. Serving Badar and Shahbaz from one deployment needs a
tenant column and RLS on it, or a second deployment. That is a fork in the road worth
taking deliberately rather than discovering later, and it is much cheaper to decide now
than after Shahbaz's data is in.

## 3.10 The "these clients will not accept an ordinary-looking CRM" bar

Muhammad's point: both Badar and Shahbaz are high earners and will judge this on how it
looks. Fair, and worth being concrete rather than saying "make it premium".
...
1. **Every page reachable in one or two clicks from the dashboard.** Worth auditing ours
   against - Create New Campaign was four clicks deep until today.
