...
and the parts that touch money need Badar's numbers first. The Settings tab already
captures those numbers so he can set them before the engine exists.
...
1. **Stamp `verified_at`** and stop treating `verified` as decoration. (`converted_at` is
   already stamped since 2026-08-08.)
...
engineering ones. All five already have a field waiting in **Settings**:
...
Plus one that has no field yet because it is a policy: **who besides Badar may clear an
